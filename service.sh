...
fake_battery_temp() {
    ...
    [ -w "/sys/class/power_supply/battery/temp" ] && echo "$TEMP_VAL" > "/sys/class/power_supply/battery/temp" 2>/dev/null
    [ -w "/sys/class/power_supply/battery/batt_temp" ] && echo "$temp_c" > "/sys/class/power_supply/battery/batt_temp" 2>/dev/null
    [ -w "/sys/class/power_supply/battery/batt_temp" ] && echo "$TEMP_VAL" > "/sys/class/power_supply/battery/batt_temp" 2>/dev/null
    [ -w "/sys/class/power_supply/battery/battery_temp" ] && echo "$temp_c" > "/sys/class/power_supply/battery/battery_temp" 2>/dev/null
    [ -w "/sys/class/power_supply/battery/battery_temp" ] && echo "$TEMP_VAL" > "/sys/class/power_supply/battery/battery_temp" 2>/dev/null
...
while true; do
    ...
    if [ "$STATUS" = "Charging" ] || [ "$STATUS" = "Full" ]; then
        if [ "$LAST_STATE" != "1" ]; then
            setprop persist.vendor.thermal.enable 0
            setprop vendor.thermal.enable 0
            setprop sys.thermal.enable 0
            stop thermal-engine
